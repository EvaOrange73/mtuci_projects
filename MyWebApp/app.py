...
def registration():
    if request.method == 'POST':
        ...
        if not name or not login or not password:
            return render_template('registration.html', name=name, login=login, password=password, first_time=False, repeat=False)

        cursor.execute('SELECT * FROM service.users WHERE login = %s', (str(login)))
        records = list(cursor.fetchall())

        if records:
            return render_template('registration.html', name=name, login=login, password=password, first_time=False, repeat=True)
    ...
    return render_template('registration.html', name="", login="", password="", first_time=True, repeat=False)
